...
shinyServer(function(input, output) {
  ...
  prepare_meta_cohd_summ <- function() {
    ...
    query <- sprintf("
                  SELECT * FROM session_meta SM,
                     ...
                     AND MCR.metric=N_DEMOG.metric
                     AND MCR.meta_key_ID=SM.meta_key_ID
                     ",input$lm,input$studyID,input$lm,input$metric)

    res <- dbGetQuery(db_conn,query)
    res
}  
  output$meta_cohd_summary <- renderTable({
    prepare_meta_cohd_summ()
