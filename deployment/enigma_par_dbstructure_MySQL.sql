...
-- Last modification date: 2017-06-28 20:06:17.637
...
CREATE TABLE feature_sets (
    session_fs_ID character(200) NOT NULL,
    fsID character(100) NOT NULL,
    metric character(100) NOT NULL,
    ROI character(100) NOT NULL,
    CONSTRAINT feature_sets_pk PRIMARY KEY (session_fs_ID,fsID,metric,ROI)
);

-- Table: feature_sets_covariates
CREATE TABLE feature_sets_covariates (
    ...
    CONSTRAINT feature_sets_covariates_pk PRIMARY KEY (session_fs_ID,fsID,row_ID)
);

-- Table: feature_sets_newregr
CREATE TABLE feature_sets_newregr (
    session_fs_ID character(200) NOT NULL,
    fsID character(100) NOT NULL,
    var character(100) NOT NULL,
    formula varchar(1000) NOT NULL,
    CONSTRAINT feature_sets_newregr_pk PRIMARY KEY (session_fs_ID,fsID,var)
);
...
CREATE TABLE session_fs (
    ...
    timestamp datetime NOT NULL,
    study_site_ID int NOT NULL,
    CONSTRAINT session_fs_pk PRIMARY KEY (session_fs_ID,fsID)
);
...
-- Reference: Table_30_session_fs (table: feature_sets_newregr)
ALTER TABLE feature_sets_newregr ADD CONSTRAINT Table_30_session_fs FOREIGN KEY Table_30_session_fs (session_fs_ID,fsID)
    REFERENCES session_fs (session_fs_ID,fsID);
...
-- Reference: feature_sets_covariates_session_fs (table: feature_sets_covariates)
ALTER TABLE feature_sets_covariates ADD CONSTRAINT feature_sets_covariates_session_fs FOREIGN KEY feature_sets_covariates_session_fs (session_fs_ID,fsID)
    REFERENCES session_fs (session_fs_ID,fsID);

-- Reference: feature_sets_session_fs (table: feature_sets)
ALTER TABLE feature_sets ADD CONSTRAINT feature_sets_session_fs FOREIGN KEY feature_sets_session_fs (session_fs_ID,fsID)
    REFERENCES session_fs (session_fs_ID,fsID);

-- Reference: linear_model_session_fs (table: linear_model)
ALTER TABLE linear_model ADD CONSTRAINT linear_model_session_fs FOREIGN KEY linear_model_session_fs (session_fs_ID,fsID)
    REFERENCES session_fs (session_fs_ID,fsID);
...
-- Reference: session_fs_sites_in_study (table: session_fs)
ALTER TABLE session_fs ADD CONSTRAINT session_fs_sites_in_study FOREIGN KEY session_fs_sites_in_study (study_site_ID)
    REFERENCES sites_in_study (study_site_ID);
